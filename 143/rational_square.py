...
def square_root_rational(x,abs_tol=Rational(1,1000)):
    ...
    """Input x a rational number from class Rational, abs_total a number
    from Rational class indicating precision of output: the square root of x"""
    ...
    num = fx
    xx = Rational(0,1)
    solution = Rational(0,1)
    
    while abs(float(solution*solution - num)) > float(abs_tol) and solution <= fx:
        
        solution = (xx + fx) / 2
        
        if solution * solution < num:
            xx = solution
